...
inline float lumaOf(const uint8_t *p) {
    return 0.299f * p[0] + 0.587f * p[1] + 0.114f * p[2];
}

inline void sampleClamped(const std::vector<uint8_t> &buf, uint32_t w, uint32_t h,
                           int x, int y, uint8_t out[4]) {
    x = std::clamp(x, 0, static_cast<int>(w) - 1);
    y = std::clamp(y, 0, static_cast<int>(h) - 1);
    const uint8_t *sp = buf.data() + (static_cast<size_t>(y) * w + x) * 4u;
    out[0] = sp[0]; out[1] = sp[1]; out[2] = sp[2]; out[3] = sp[3];
}

/**
 * CPU mirror of the GPU antiAlias()/fxaa()/smaaLite() functions in
 * gpu_image_enhancement.cpp, kept algorithmically in sync so the live preview
 * matches the on-device render-loop result. mode: 0 = off, 1 = FXAA, 2 = SMAA
 * (lightweight single-pass approximation — see the GPU shader comment for why
 * this isn't the full multi-pass SMAA algorithm).
 */
void applyAntiAliasCpu(std::vector<uint8_t> &buf, uint32_t w, uint32_t h, int mode) {
    if (mode <= 0 || w < 3 || h < 3) return;
    std::vector<uint8_t> src = buf; // read from a stable copy, write into buf

    for (uint32_t yu = 0; yu < h; ++yu) {
        for (uint32_t xu = 0; xu < w; ++xu) {
            const int x = static_cast<int>(xu), y = static_cast<int>(yu);
            uint8_t cC[4], cN[4], cS[4], cE[4], cW[4];
            sampleClamped(src, w, h, x, y, cC);
            sampleClamped(src, w, h, x, y - 1, cN);
            sampleClamped(src, w, h, x, y + 1, cS);
            sampleClamped(src, w, h, x + 1, y, cE);
            sampleClamped(src, w, h, x - 1, y, cW);
            const float lC = lumaOf(cC), lN = lumaOf(cN), lS = lumaOf(cS);
            const float lE = lumaOf(cE), lW = lumaOf(cW);

            uint8_t *dp = buf.data() + (static_cast<size_t>(yu) * w + xu) * 4u;

            if (mode == 1) { // FXAA
                uint8_t cNW[4], cNE[4], cSW[4], cSE[4];
                sampleClamped(src, w, h, x - 1, y - 1, cNW);
                sampleClamped(src, w, h, x + 1, y - 1, cNE);
                sampleClamped(src, w, h, x - 1, y + 1, cSW);
                sampleClamped(src, w, h, x + 1, y + 1, cSE);
                const float lNW = lumaOf(cNW), lNE = lumaOf(cNE);
                const float lSW = lumaOf(cSW), lSE = lumaOf(cSE);
                const float lMin = std::min({lC, lN, lS, lE, lW});
                const float lMax = std::max({lC, lN, lS, lE, lW});
                const float range = lMax - lMin;
                const float threshold = std::max(8.0f, lMax * 0.125f); // 8-bit scale of 0.0312
                if (range < threshold) continue;

                const float edgeHoriz = std::fabs((lNW + lSW) - 2.0f * lW) + 2.0f * std::fabs((lN + lS) - 2.0f * lC) + std::fabs((lNE + lSE) - 2.0f * lE);
                const float edgeVert  = std::fabs((lNW + lNE) - 2.0f * lN) + 2.0f * std::fabs((lW + lE) - 2.0f * lC) + std::fabs((lSW + lSE) - 2.0f * lS);
                const bool horizontal = edgeHoriz >= edgeVert;
                const uint8_t *c1 = horizontal ? cN : cW;
                const uint8_t *c2 = horizontal ? cS : cE;
                const float l1 = horizontal ? lN : lW;
                const float l2 = horizontal ? lS : lE;

                const float gradient = std::fabs(l1 - l2);
                const float blend = std::clamp(gradient / std::max(range, 1e-4f), 0.0f, 1.0f) * 0.5f;
                float blended[3], avg3x3[3];
                for (int k = 0; k < 3; ++k) {
                    const float mixC1C2 = 0.5f * (c1[k] + c2[k]);
                    blended[k] = cC[k] + (mixC1C2 - cC[k]) * blend;
                    avg3x3[k] = (cC[k] + cN[k] + cS[k] + cE[k] + cW[k] + cNW[k] + cNE[k] + cSW[k] + cSE[k]) / 9.0f;
                }
                const float subpixBlend = std::clamp(range * 2.0f / 255.0f, 0.0f, 1.0f) * 0.25f;
                for (int k = 0; k < 3; ++k)
                    dp[k] = static_cast<uint8_t>(std::clamp(blended[k] + (avg3x3[k] - blended[k]) * subpixBlend, 0.0f, 255.0f));
                dp[3] = cC[3];
            } else { // SMAA (lightweight single-pass approximation)
                const float deltaV = std::max(std::fabs(lC - lN), std::fabs(lC - lS));
                const float deltaH = std::max(std::fabs(lC - lE), std::fabs(lC - lW));
                const float threshold = 0.05f * 255.0f;
                if (deltaV < threshold && deltaH < threshold) continue;

                float vAccum[3] = {static_cast<float>(cC[0]), static_cast<float>(cC[1]), static_cast<float>(cC[2])};
                float hAccum[3] = {static_cast<float>(cC[0]), static_cast<float>(cC[1]), static_cast<float>(cC[2])};
                float vWeight = 1.0f, hWeight = 1.0f;
                for (int i = 1; i <= 4; ++i) {
                    uint8_t up[4], dn[4], lf[4], rt[4];
                    sampleClamped(src, w, h, x, y - i, up);
                    sampleClamped(src, w, h, x, y + i, dn);
                    sampleClamped(src, w, h, x - i, y, lf);
                    sampleClamped(src, w, h, x + i, y, rt);
                    const float falloff = 1.0f / static_cast<float>(i + 1);
                    for (int k = 0; k < 3; ++k) {
                        vAccum[k] += (up[k] + dn[k]) * falloff;
                        hAccum[k] += (lf[k] + rt[k]) * falloff;
                    }
                    vWeight += 2.0f * falloff;
                    hWeight += 2.0f * falloff;
                }
                const float wV = deltaV / std::max(deltaV + deltaH, 1e-4f);
                const float coverage = std::clamp(std::max(deltaV, deltaH) * 3.0f / 255.0f, 0.0f, 0.7f);
                for (int k = 0; k < 3; ++k) {
                    const float vBlur = vAccum[k] / vWeight;
                    const float hBlur = hAccum[k] / hWeight;
                    const float directional = hBlur + (vBlur - hBlur) * wV;
                    dp[k] = static_cast<uint8_t>(std::clamp(cC[k] + (directional - cC[k]) * coverage, 0.0f, 255.0f));
                }
                dp[3] = cC[3];
            }
        }
...
Java_com_firstt175_deepdrop_session_NativeBridge_initContext(
        ...
        jint aiEngine, jint frameTransferMode) {
    const std::string cache = jstring_to_std(env, cacheDir);
    ...
    const lsfg_android::RenderLoopConfig cfg{
        ...
        .frameTransferMode = static_cast<lsfg_android::FrameTransferMode>(std::clamp(static_cast<int>(frameTransferMode), 0, 2)),
    };
...
extern "C" JNIEXPORT void JNICALL
Java_com_firstt175_deepdrop_session_NativeBridge_setFrameTransferMode(
        JNIEnv * /*env*/, jobject /*thiz*/, jint mode) {
    lsfg_android::setFrameTransferMode(static_cast<int>(mode));
}
...
Java_com_firstt175_deepdrop_session_NativeBridge_processImageEnhancementPreview(
        ...
        jfloat renderResolutionScale, jint antiAliasMode) {
    ...
    // Match the GPU pipeline's order: anti-aliasing runs on the raw captured
    // samples first, then contrast/saturation enhancement, then upscale.
    applyAntiAliasCpu(work, rw, rh, std::clamp(static_cast<int>(antiAliasMode), 0, 2));
...
extern "C" JNIEXPORT void JNICALL
Java_com_firstt175_deepdrop_session_NativeBridge_setAntiAliasMode(
        JNIEnv * /*env*/, jobject /*thiz*/, jint mode) {
    lsfg_android::setAntiAliasMode(static_cast<int>(mode));
}
