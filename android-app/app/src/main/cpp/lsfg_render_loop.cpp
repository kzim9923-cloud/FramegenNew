...
struct State {
    ...
    std::atomic<float> imageEnhancementStrength{0.35f};
...
bool blitOutputToSwapchain(const AhbImage &src, VkSemaphore framegenDoneSemaphore = VK_NULL_HANDLE) {
    ...
    // frames when the presentation queue is temporarily busy. NO_WAIT scheduling
    ...
    if (gpuEnhance) {
        const float strength = g.imageEnhancementStrength.load(std::memory_order_relaxed);
...
// buffer, a 4-neighbour unsharp pass, then contrast/saturation. It runs immediately
// before presentation, so changing the controls affects the next frame without a
// session restart. Non-RGBA AHBs are left untouched.
bool applyCpuImageEnhancement(const AhbImage &out) {
    ...
    const float strength = std::clamp(g.imageEnhancementStrength.load(std::memory_order_relaxed), 0.0f, 1.0f);
    ...
    auto sample = [&](int x, int y, int c) -> float {
        x = std::clamp(x, 0, static_cast<int>(desc.width) - 1);
        y = std::clamp(y, 0, static_cast<int>(desc.height) - 1);
        return static_cast<float>(buf[(static_cast<size_t>(y) * desc.width + static_cast<size_t>(x)) * 4u + c]);
    };
    for (uint32_t y = 0; y < desc.height; ++y) {
        for (uint32_t x = 0; x < desc.width; ++x) {
            const size_t i = (static_cast<size_t>(y) * desc.width + x) * 4u;
            float r = sample(static_cast<int>(x), static_cast<int>(y), 0);
            float gg = sample(static_cast<int>(x), static_cast<int>(y), 1);
            float b = sample(static_cast<int>(x), static_cast<int>(y), 2);
            if (strength > 0.001f) {
                const float nr = (sample(static_cast<int>(x)-1, static_cast<int>(y), 0) + sample(static_cast<int>(x)+1, static_cast<int>(y), 0) +
                                  sample(static_cast<int>(x), static_cast<int>(y)-1, 0) + sample(static_cast<int>(x), static_cast<int>(y)+1, 0)) * 0.25f;
                const float ng = (sample(static_cast<int>(x)-1, static_cast<int>(y), 1) + sample(static_cast<int>(x)+1, static_cast<int>(y), 1) +
                                  sample(static_cast<int>(x), static_cast<int>(y)-1, 1) + sample(static_cast<int>(x), static_cast<int>(y)+1, 1)) * 0.25f;
                const float nb = (sample(static_cast<int>(x)-1, static_cast<int>(y), 2) + sample(static_cast<int>(x)+1, static_cast<int>(y), 2) +
                                  sample(static_cast<int>(x), static_cast<int>(y)-1, 2) + sample(static_cast<int>(x), static_cast<int>(y)+1, 2)) * 0.25f;
                r += (r - nr) * strength;
                gg += (gg - ng) * strength;
                b += (b - nb) * strength;
            }
...
void workerThread() {
    ...
    while (true) {
        ...
        if (!framegenInputsBusy && (g.framesCopied == 0 || pairingResetThisFrame)) {
            ...
            // capture's generation was bypassed (backlog, cpuFallbackGenInFlight,
            // ...) and framegen's internal frameIdx
...
void setImageEnhancement(bool enabled, int backend, float strength, float contrast, float saturation) {
    ...
    g.imageEnhancementStrength.store(std::clamp(strength, 0.0f, 1.0f), std::memory_order_relaxed);
    ...
    LOGI("Image enhancement live: enabled=%d backend=%d strength=%.2f contrast=%.2f saturation=%.2f",
         enabled ? 1 : 0, std::clamp(backend,0,1),
         std::clamp(strength,0.0f,1.0f), std::clamp(contrast,0.5f,1.5f),
