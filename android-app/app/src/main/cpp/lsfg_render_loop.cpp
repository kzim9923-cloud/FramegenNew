...
struct State {
    ...
    // jobFrameId == currentEpoch EXACTLY (any epoch drift = dropped, this is
    ...
    // processing. Written under genMu alongside genJobStartedAt/genJobFrameId
...
void workerThread() {
    ...
    while (true) {
        State::PendingFrame pendingFrame{};
        {
            ...
            // Always restore REAL-frame order before consumption. Capture backends
            // only enqueue {buffer,timestamp,id}; ordering belongs here.
        ...
        // This is the single ordering point for both modes.
        ...
        // Scheduling policy is intentionally only the two user-selected modes.
        // No dynamic bypass, preflight, pacing estimate, queue-depth policy or
        // stale-generation tolerance participates in admission anymore.
        const bool generationPreflightOk = generationAllowed;
...
void pushFrame(AHardwareBuffer *ahb, int64_t timestampNs, uint64_t frameId) {
    ...
    AHardwareBuffer_acquire(ahb);
    {
        ...
        // real-frame ID. No dropping, pacing, generation admission or latency
        // decisions happen in the capture path.
...
// default reproduces the original strict jobFrameId==currentEpoch behaviour
