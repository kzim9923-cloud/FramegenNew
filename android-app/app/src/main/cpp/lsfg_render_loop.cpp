...
struct State {
    ...
    PostProcessMode postProcessMode = PostProcessMode::OFF;
    float postProcessSharpness = 0.0f;
    CasRcasPostProcess postProcess{};
...
bool blitOutputToSwapchain(const AhbImage &src, VkSemaphore framegenDoneSemaphore = VK_NULL_HANDLE) {
    ...
    const bool usePost = g.postProcessMode != PostProcessMode::OFF &&
                         g.postProcess.ready();

    bool postProcessRecorded = false;
    if (usePost) {
        if (!g.postProcess.record(g.vk, cb, src.image, g.swap.images[imageIdx],
                                  src.extent, g.swap.extent,
                                  g.postProcessMode, g.postProcessSharpness)) {
            LOGW("CAS/RCAS post-process failed; falling back to plain GPU blit");
            const VkImageBlit region{
                .srcSubresource={VK_IMAGE_ASPECT_COLOR_BIT,0,0,1},
                .srcOffsets={{0,0,0},{static_cast<int32_t>(src.extent.width),static_cast<int32_t>(src.extent.height),1}},
                .dstSubresource={VK_IMAGE_ASPECT_COLOR_BIT,0,0,1},
                .dstOffsets={{0,0,0},{static_cast<int32_t>(g.swap.extent.width),static_cast<int32_t>(g.swap.extent.height),1}}};
            g.vk.fn.vkCmdBlitImage(cb,src.image,VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL,
                                   g.swap.images[imageIdx],VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL,
                                   1,&region,VK_FILTER_LINEAR);
        } else {
            postProcessRecorded = true;
        }
        if (!loggedBlitMode.exchange(true,std::memory_order_relaxed)) {
            LOGI("post-process: %s sharpness=%.2f (%ux%u -> %ux%u)",
                 g.postProcessMode == PostProcessMode::RCAS ? "RCAS" : "CAS",
                 g.postProcessSharpness, src.extent.width, src.extent.height,
        ...
        g.vk.fn.vkCmdCopyImage(cb,src.image,VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL,
                               g.swap.images[imageIdx],VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL,
                               1,&region);
    } else {
        const VkImageBlit region{
    ...
    VkImageMemoryBarrier srcRelease = makeImageBarrier(src.image,
        postProcessRecorded ? 0 : VK_ACCESS_TRANSFER_READ_BIT, 0,
        postProcessRecorded ? VK_IMAGE_LAYOUT_GENERAL : VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL,
...
int initRenderLoop(const char *cacheDir, const RenderLoopConfig &cfg) {
    ...
    if (rc != kOk) {
        ...
        g.postProcess.shutdown(g.vk);
    destroy_session(g.vk);
        return kRenderLoopSessionFailed;
    }

    g.postProcessMode = static_cast<PostProcessMode>(std::clamp(cfg.postProcessMode, 0, 2));
    g.postProcessSharpness = std::clamp(cfg.postProcessSharpness, 0.0f, 1.0f);
    if (g.postProcessMode != PostProcessMode::OFF) {
        if (!g.postProcess.init(g.vk)) {
            LOGW("CAS/RCAS post-process unavailable; continuing without it");
            g.postProcessMode = PostProcessMode::OFF;
        }
    }
