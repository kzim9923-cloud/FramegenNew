...
struct State {
    ...
    // Anti-aliasing applied to source samples before enhancement/upscale.
    // 0 = OFF, 1 = FXAA, 2 = SMAA (lightweight single-pass approximation).
    std::atomic<int> antiAliasMode{0};
...
bool blitOutputToSwapchain(const AhbImage &src, VkSemaphore framegenDoneSemaphore = VK_NULL_HANDLE) {
    ...
    const bool enhancementRequested =
        (g.imageEnhancementEnabled.load(std::memory_order_relaxed) &&
         g.imageEnhancementBackend.load(std::memory_order_relaxed) == 1) ||
        g.antiAliasMode.load(std::memory_order_relaxed) > 0;
...
void workerThread() {
    ...
    while (true) {
        ...
        const bool zeroCopyMode = g.frameTransferMode.load(std::memory_order_relaxed)
            == static_cast<int>(FrameTransferMode::ZERO_COPY);
        if (zeroCopyMode) {
        ...
        auto enqueueCurrentReal = [&]() {
            const AhbImage &realImage = zeroCopyMode
                ? src
                : (framegenInputsBusy ? src : g.inSlot[newSlot]);
            enqueueFinalFrame(
                realImage,
...
void setAntiAliasMode(int mode) {
    ...
    g.antiAliasMode.store(clamped, std::memory_order_relaxed);
    static const char *kNames[3] = {"OFF", "FXAA", "SMAA"};
    LOGI("Anti-aliasing live: mode=%d (%s)", clamped, kNames[clamped]);
}
