...
struct State {
    ...
    // Real FIFO frame queue. Every capture admitted by pushFrame() is kept
    // and processed in order — the worker never skips a queued capture and
    // every one of them goes through generation (see generationAllowed).
    // The queue is bounded by g.frameQueueCapacity purely as a memory/
    // latency safety valve for a worker that falls arbitrarily far behind:
    // when full, pushFrame() drops only the single oldest NOT-YET-PROCESSED
    // entry to admit the newest capture. The frame currently popped and
    // being worked on is never in this deque, so it's never affected.
    struct PendingFrame {
    ...
    // Set on manual bypass toggling or a queue-overflow drop, so no frame
    // from before the discontinuity can ever be used as an interpolation
    ...
    // Frame generation is mandatory: every real capture that reaches the
    // worker always goes through generation (when a backend is loaded and
    // the structural pipeline state allows it — see generationAllowed).
    // These fields are pure measurement/telemetry (HUD queue/latency stats,
    // resolution-normalized cost model) — they are never used to skip or
    ...
    // --- Frame queue (replaces the old evict-on-push single-slot cache) ---
    // Every admitted capture is retained and processed in order; nothing is
    // silently dropped anymore. `frameQueueCapacity` is user-configurable
    // from the overlay (see setFrameQueueCapacity / NativeBridge). If the
    // worker ever falls behind far enough to hit the cap, pushFrame() drops
    // only the single OLDEST queued (not-yet-processed) frame to admit the
    // newest one — it never wipes the whole queue or forces a bypass.
    std::atomic<int32_t> frameQueueCapacity{8};
    std::atomic<uint64_t> queueOverflowDroppedCount{0};
...
void genWorkerThread() {
    while (true) {
        ...
        // Feed the same cost-telemetry model the main thread uses (HUD
        // stats / resolution-normalized cost model) — measurement only, it
...
void workerThread() {
    ...
    while (true) {
        ...
        //
        // NO ADAPTIVE/LOAD-BASED BYPASS: generation is mandatory, not
        // optional idle work. Every queued real capture goes through
        // generation — there is no dynamic backlog bypass, no "queued real
        // frame" pass-through, and no auto-latency bypass. Backlog is
        // absorbed by the frame queue itself (see pushFrame /
        // frameQueueCapacity) instead of by skipping generation. Only
        // genuinely structural conditions below can prevent generation for
        // a given frame (no backend loaded, first-frame bootstrap, input
        // buffers still in flight, a CPU-fallback publish job still
        // draining) — these are correctness requirements, not a policy
        // choice, and are unrelated to load or timing.
        //
        // Still track capture cadence / generation cost as pure telemetry
        // (HUD stats, resolution-normalized cost model) — it no longer
        // gates anything.
        ...
        (void)captureIntervalNs; // telemetry only
        ...
        const bool generationAllowed =
            ...
            // reuse yet — this is a hardware buffer-reuse constraint, not a
            // load-shedding choice. The pending real frame is queued and
            // will get its turn (and its generation) on the very next cycle.
            !g.cpuFallbackGenInFlight.load(std::memory_order_acquire);

        // Any skipped REAL frame here is a discontinuity for interpolation
        // (manual bypass, or one of the structural conditions above). The
        // current REAL frame remains visible, but the next generation must
        // re-anchor from a fresh REAL pair.
        ...
        // Generation preflight now only checks genuine hardware capability,
        // never cost/backlog. On devices without a working exported
        // completion semaphore, publishing falls back to a background
        // CPU-waitIdle job (see useCpuWaitIdleFallback below) instead of
        // being skipped — that path exists specifically so this case still
        // runs generation for every frame without stalling capture.
...
void pushFrame(AHardwareBuffer *ahb, int64_t timestampNs) {
    ...
    AHardwareBuffer_acquire(ahb);
    {
        ...
        // REAL FIFO QUEUE: every admitted capture is retained and processed
        // in order, and every one of them goes through generation once the
        // worker gets to it (see generationAllowed in workerThread). This
        // used to evict/clear the whole queue on every push, which silently
        // dropped not-yet-processed real frames and forced a raw-blit
        // bypass for whichever frame was mid-flight. That's gone: the queue
        // now only sheds load at a hard capacity limit, and even then it
        // drops just the single oldest waiting entry (never the newest, and
        // never the frame the worker already popped and is processing).
        const int32_t capacity = std::max<int32_t>(
            1, g.frameQueueCapacity.load(std::memory_order_relaxed));
        while (static_cast<int32_t>(g.pendingFrames.size()) >= capacity) {
            auto &oldest = g.pendingFrames.front();
            if (oldest.ahb != nullptr) {
                AHardwareBuffer_release(oldest.ahb);
            }
            g.pendingFrames.pop_front();
            g.queueOverflowDroppedCount.fetch_add(1, std::memory_order_relaxed);
            // A dropped queued frame is still a discontinuity for
            // interpolation — re-anchor on the next processed real pair.
            g.pairingResetPending.store(true, std::memory_order_release);
            LOGW("pushFrame: frame queue at capacity (%d); dropped oldest queued frame", capacity);
        }
...
// Set the maximum number of not-yet-processed captures the frame queue will
// hold before it starts dropping the oldest waiting entry to admit new
// captures (see pushFrame). Larger values absorb longer worker stalls
// without discarding frames, at the cost of more retained AHardwareBuffer
// references (memory) and more queueing latency if the worker is
// persistently behind. Exposed to the overlay as a normal tunable, same as
// multiplier/flowScale/pacing. Safe to call from any thread.
void setFrameQueueCapacity(int32_t capacity) {
    const int32_t clamped = std::clamp<int32_t>(capacity, 1, 64);
    g.frameQueueCapacity.store(clamped, std::memory_order_relaxed);
    LOGI("Frame queue capacity set to %d", clamped);
}

int32_t getFrameQueueCapacity() {
    return g.frameQueueCapacity.load(std::memory_order_relaxed);
}

uint64_t getQueueOverflowDroppedCount() {
    return g.queueOverflowDroppedCount.load(std::memory_order_relaxed);
}
