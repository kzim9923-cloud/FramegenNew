...
void workerThread() {
    // Keep the transfer-mode switch as the single source of truth for zero-copy.\n    // Older code referenced a removed standalone zeroCopyMode flag.\n    const bool zeroCopyMode =\n        g.frameTransferMode.load(std::memory_order_relaxed) ==\n        static_cast<int>(FrameTransferMode::ZERO_COPY);\n    // Do not set CPU affinity here. The worker inherits the process/default
