...
struct State {
    ...
    // setPacingParams() and read atomically by the generation-cost estimator.
    ...
    // from Settings. Set via setDynamicBypassEnabled/setRealFrameFirstEnabled.
    ...
    std::atomic<bool> realFrameFirstEnabled{false};
    // Generation preflight's safety-window check (see generationPreflightOk):
    // rejects a generation whose projected cost would already eat too much of
    // the source capture interval. OFF by default — user must enable it from
    // Settings; ratio is user-tunable via setPreflightSafetyWindowRatio().
    std::atomic<bool> preflightSafetyWindowEnabled{false};
    std::atomic<double> preflightSafetyWindowRatio{0.65}; // fraction of captureIntervalNs
    // genWorker's "dropping stale generated outputs" LOGI (see genWorkerThread)
    // fires on every epoch-invalidated generation and can spam logcat under
    // normal operation (bypass toggles, resizes, resolution changes). OFF by
    // default — user must enable it from Settings via
    // setStaleDropLoggingEnabled() when they actually want to diagnose it.
    std::atomic<bool> staleDropLoggingEnabled{false};
    // Real behavior control for genWorkerThread's REAL->GEN ordering guard:
    // by default a generated output is only ever published when
    // jobEpoch == currentEpoch EXACTLY (any epoch drift = dropped, this is
    // the original strict/safe behaviour and stays OFF/0 by default). When
    // the user explicitly enables this from Settings, a generation may still
    // be published as long as it isn't more than staleGenerationToleranceEpochs
    // behind the current epoch — trading a small amount of potential visual
    // staleness for fewer dropped generations during rapid epoch churn
    // (bypass toggles, resizes). User-tunable via
    // setStaleGenerationToleranceEnabled()/setStaleGenerationToleranceEpochs().
    std::atomic<bool> staleGenerationToleranceEnabled{false};
    std::atomic<int> staleGenerationToleranceEpochs{0}; // clamped [0, 10]
...
void recordGenerationCostSample(int64_t generationNs, uint64_t pixelCount) {
    ...
    const float alpha = std::clamp(g.pacingEmaAlpha.load(std::memory_order_relaxed), 0.05f, 0.5f);
    const float outlierRatio = std::clamp(g.pacingOutlierRatio.load(std::memory_order_relaxed), 2.0f, 8.0f);
    // Reject extreme generation-cost spikes before feeding the adaptive model.
    // The first sample always seeds the model; later samples above
    // baseline*outlierRatio are ignored rather than poisoning admission control.
    {
        const int64_t old = g.emaGenerationNs.load(std::memory_order_relaxed);
        if (old <= 0) {
            g.emaGenerationNs.store(generationNs, std::memory_order_relaxed);
        } else if (static_cast<double>(generationNs) <= static_cast<double>(old) * outlierRatio) {
            const double next = static_cast<double>(old) +
                static_cast<double>(alpha) * (static_cast<double>(generationNs) - static_cast<double>(old));
            g.emaGenerationNs.store(static_cast<int64_t>(std::max(1.0, next)), std::memory_order_relaxed);
        }
    }
    ...
    if (old <= 0) {
        g.emaGenNsPerMegapixel.store(sampleNsPerMp, std::memory_order_relaxed);
    } else if (static_cast<double>(sampleNsPerMp) <= static_cast<double>(old) * outlierRatio) {
        const double next = static_cast<double>(old) +
            static_cast<double>(alpha) * (static_cast<double>(sampleNsPerMp) - static_cast<double>(old));
        g.emaGenNsPerMegapixel.store(static_cast<int64_t>(std::max(1.0, next)), std::memory_order_relaxed);
    }
}
...
void genWorkerThread() {
    while (true) {
        ...
        // Epochs only ever move forward, so this is how far behind the
        // current epoch this job's output already is. 0 == exact match.
        const uint64_t epochsBehind =
            (currentEpoch >= jobEpoch) ? (currentEpoch - jobEpoch) : 0;
        const uint64_t toleranceEpochs =
            g.staleGenerationToleranceEnabled.load(std::memory_order_relaxed)
                ? static_cast<uint64_t>(
                      g.staleGenerationToleranceEpochs.load(std::memory_order_relaxed))
                : 0; // OFF by default => tolerance 0 => original strict jobEpoch==currentEpoch behaviour
        const bool generationStillValid =
            publishOutputs &&
            jobEpoch != 0 &&
            epochsBehind <= toleranceEpochs &&
        ...
        } else if (g.staleDropLoggingEnabled.load(std::memory_order_relaxed)) {
            LOGI("genWorker: dropping stale generated outputs jobEpoch=%llu currentEpoch=%llu epochsBehind=%llu toleranceEpochs=%llu bypass=%d",
                 static_cast<unsigned long long>(jobEpoch),
                 static_cast<unsigned long long>(currentEpoch),
                 static_cast<unsigned long long>(epochsBehind),
                 static_cast<unsigned long long>(toleranceEpochs),
...
void workerThread() {
    ...
    while (true) {
        ...
        // OFF by default (see g.dynamicBypassEnabled) — the user must turn
        // this on from Settings before backlog pressure is allowed to skip
        // generation automatically; the budget multiplier below is also
        // user-tunable via setDynamicBypassBudgetMultiplier().
        bool dynamicBypass = false;
        if (g.dynamicBypassEnabled.load(std::memory_order_relaxed) &&
            emaGenerationNs > 0 && captureIntervalNs > 0) {
            const int64_t queuedWorkNs =
                static_cast<int64_t>(queuedBehind) * emaGenerationNs;
            const int64_t baseIntervalNs = std::max<int64_t>(captureIntervalNs, 1'000'000LL);
            const double multiplier = g.dynamicBypassBudgetMultiplier.load(std::memory_order_relaxed);
            const int64_t latencyBudgetNs =
                static_cast<int64_t>(static_cast<double>(baseIntervalNs) * multiplier);
        ...
        // OFF by default (see g.realFrameFirstEnabled) — the user must turn
        // this on from Settings before a queued newer real frame is allowed
        // to skip generation automatically.
        bool newerRealFrameWaiting = false;
        if (g.realFrameFirstEnabled.load(std::memory_order_relaxed)) {
            std::lock_guard<std::mutex> qlock(g.mu);
        ...
        if (generationPreflightOk) {
            ...
            const int64_t sourceIntervalNs = captureIntervalNs > 0
                ? captureIntervalNs
                : 0;
            // OFF by default (see g.preflightSafetyWindowEnabled) — the user
            // must enable this from Settings before a generation projected to
            // eat too much of the capture interval gets rejected here. Ratio
            // is user-tunable via setPreflightSafetyWindowRatio().
            if (g.preflightSafetyWindowEnabled.load(std::memory_order_relaxed)) {
                const double ratio = g.preflightSafetyWindowRatio.load(std::memory_order_relaxed);
                const int64_t safetyWindowNs = sourceIntervalNs > 0
                    ? static_cast<int64_t>(static_cast<double>(sourceIntervalNs) * ratio)
                    : 0;
                if (genEstimateNs > 0 && safetyWindowNs > 0 && genEstimateNs >= safetyWindowNs) {
                    generationPreflightOk = false;
                }
            }
            // Never start generation when the current pair has no measured
            // timing yet and there is no recent generation history to justify
            // the cost — always on, this is a bootstrap safety, not a policy
            // (there is no cost estimate to preflight against yet).
            if (sourceIntervalNs <= 0 && genEstimateNs <= 0) {
                generationPreflightOk = false;
            }
            // Never start generation when the queue already contains another
            // REAL frame. Same underlying queue check as the REAL-FRAME-FIRST
            // policy above (g.realFrameFirstEnabled) — shares its toggle so
            // enabling/disabling that one setting controls both places
            // consistently instead of leaving a second hidden copy of the
            // same policy always-on underneath it.
            if (g.realFrameFirstEnabled.load(std::memory_order_relaxed) && queuedBehind > 0) {
                generationPreflightOk = false;
            }
            // (Previously this rejected generation outright on any device
            // without a working exported completion semaphore, on the theory
            // that CPU waitIdle() would stall the real-frame lane. It no
            // longer does: waitIdle() always runs on genWorkerThread now, off
            // the capture/present hot path, so there is no such stall to
            // avoid — see useCpuWaitIdleFallback below.)
        }
        ...
        if (runFramegen || runAi) {
            ...
            if (useCpuWaitIdleFallback) {
                ...
                g.cpuFallbackGenInFlight.store(true, std::memory_order_release);
                {
                    ...
                    // genWorkerThread's own jobEpoch==currentEpoch/bypass/
...
int initRenderLoop(const char *cacheDir, const RenderLoopConfig &cfg) {
    ...
    g.hdr = cfg.hdr;
    g.pacingEmaAlpha.store(std::clamp(cfg.emaAlpha, 0.05f, 0.5f), std::memory_order_relaxed);
    g.pacingOutlierRatio.store(std::clamp(cfg.outlierRatio, 2.0f, 8.0f), std::memory_order_relaxed);
...
void pushFrame(AHardwareBuffer *ahb, int64_t timestampNs) {
    ...
    AHardwareBuffer_acquire(ahb);
    {
        ...
        // Never let old captures accumulate unbounded while the worker is
        // busy. A queued AHardwareBuffer holds a reference to a potentially
        // large graphics allocation, so retaining a backlog is both a
        // latency and memory pressure problem. How many are allowed to wait
        // before the oldest get evicted is user-tunable via
        // setMaxQueuedRealFrames() (default 1, reproducing the original
        // "queue bounded to one waiting frame" behaviour).
        //
        ...
        if (g.pendingFrames.size() >= maxQueued) {
            size_t evicted = 0;
            while (g.pendingFrames.size() >= maxQueued) {
                auto &stale = g.pendingFrames.front();
            ...
            LOGI("pushFrame: evicted %zu stale queued frame(s) (max=%zu)", evicted, maxQueued);
        }
...
// User control for the REAL-FRAME-FIRST auto-bypass (see
// g.realFrameFirstEnabled at its declaration). OFF by default — must be
// explicitly enabled from Settings.
void setRealFrameFirstEnabled(bool enabled) {
    g.realFrameFirstEnabled.store(enabled, std::memory_order_relaxed);
    LOGI("Real-frame-first auto-bypass: %s", enabled ? "enabled" : "disabled");
}

// User control for the generation-preflight safety-window check (see
// generationPreflightOk). OFF by default — must be explicitly enabled.
void setPreflightSafetyWindowEnabled(bool enabled) {
    g.preflightSafetyWindowEnabled.store(enabled, std::memory_order_relaxed);
    LOGI("Preflight safety-window check: %s", enabled ? "enabled" : "disabled");
}

// User control for genWorker's "dropping stale generated outputs" LOGI (see
// genWorkerThread). OFF by default — must be explicitly enabled from
// Settings when diagnosing epoch-invalidated generations.
void setStaleDropLoggingEnabled(bool enabled) {
    g.staleDropLoggingEnabled.store(enabled, std::memory_order_relaxed);
    LOGI("Stale-generation-drop logging: %s", enabled ? "enabled" : "disabled");
}

// Real behavior control (see genWorkerThread's generationStillValid): OFF by
// default reproduces the original strict jobEpoch==currentEpoch behaviour
// (any epoch drift is dropped). Enabling this lets a generation still be
// published up to staleGenerationToleranceEpochs behind the current epoch.
void setStaleGenerationToleranceEnabled(bool enabled) {
    g.staleGenerationToleranceEnabled.store(enabled, std::memory_order_relaxed);
    LOGI("Stale-generation publish tolerance: %s", enabled ? "enabled" : "disabled");
}

// How many epochs behind currentEpoch a generation may be and still get
// published, once enabled via setStaleGenerationToleranceEnabled(). Clamped
// to [0, 10]; 0 is equivalent to the original strict exact-match behaviour.
void setStaleGenerationToleranceEpochs(int epochs) {
    const int clamped = std::clamp(epochs, 0, 10);
    g.staleGenerationToleranceEpochs.store(clamped, std::memory_order_relaxed);
    LOGI("Stale-generation publish tolerance epochs: %d", clamped);
}

// Fraction of the source capture interval a projected generation cost may
// consume before the safety-window check rejects it. Only has an effect
// while enabled via setPreflightSafetyWindowEnabled(). Clamped to [0.1, 1.0].
void setPreflightSafetyWindowRatio(double ratio) {
    const double clamped = std::clamp(ratio, 0.1, 1.0);
    g.preflightSafetyWindowRatio.store(clamped, std::memory_order_relaxed);
    LOGI("Preflight safety-window ratio set to %.2f", clamped);
}
...
void setPacingParams(float emaAlpha, float outlierRatio) {
    const float alpha = std::clamp(emaAlpha, 0.05f, 0.5f);
    const float ratio = std::clamp(outlierRatio, 2.0f, 8.0f);
    g.pacingEmaAlpha.store(alpha, std::memory_order_relaxed);
    g.pacingOutlierRatio.store(ratio, std::memory_order_relaxed);
    LOGI("Pacing params applied: emaAlpha=%.3f outlierRatio=%.2f", alpha, ratio);
}
