...
float loadR(ivec2 q) { return imageLoad(img, q).r; }
float loadG(ivec2 q) { return imageLoad(img, q).g; }
float loadB(ivec2 q) { return imageLoad(img, q).b; }
...
void main() {
    ...
    // Strength is the enhancement intensity and modulates the requested
    // contrast/saturation around their neutral values.
    float intensity = clamp(p.strength, 0.0, 1.0);
    float effectiveContrast = 1.0 + (p.contrast - 1.0) * intensity;
    float effectiveSaturation = 1.0 + (p.saturation - 1.0) * intensity;
    center.rgb = (center.rgb - 0.5) * effectiveContrast + 0.5;

    float luma = dot(center.rgb, vec3(0.2126, 0.7152, 0.0722));
    center.rgb = luma + (center.rgb - luma) * effectiveSaturation;
...
bool GpuImageEnhancement::record(VulkanSession &vk, VkCommandBuffer cb, VkImage image,
                                 VkExtent2D extent, float strength, float contrast,
                                 float saturation) {
    ...
    const std::array<float, 3> params{
