...
enum class FrameTransferMode : int { CPU_COPY = 0, GPU_COPY = 1, ZERO_COPY = 2 };
...
struct RenderLoopConfig {
    ...
    int aiEngine = 0;
    // REAL capture -> framegen input transfer. 0=CPU copy, 1=GPU copy, 2=zero-copy (experimental).
    FrameTransferMode frameTransferMode = FrameTransferMode::GPU_COPY;
};
...
// Edge anti-aliasing applied to source samples before the enhancement/upscale
// pass. mode: 0 = OFF, 1 = FXAA (fast, luma edge-detect + directional blend),
// 2 = SMAA (lightweight single-pass edge-aware directional blur approximating
// SMAA's distance-to-edge coverage; not the full multi-pass SMAA with area/search
// lookup textures, which isn't practical inside this single compute pass).
// Values outside [0,2] clamp to OFF. Safe to call from any thread.
...
// REAL capture -> framegen input transfer mode: 0=CPU, 1=GPU, 2=zero-copy experimental.
void setFrameTransferMode(int mode);
// Automatic GPU-saving bypass. The deadline is measured from REAL capture time;
