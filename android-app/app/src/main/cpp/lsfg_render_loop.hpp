...
//
// The internal queue is a real FIFO: every capture that gets in is retained
// and processed in order, and every one of them goes through generation —
// there is no load-based or timing-based bypass of generation for an
// individual capture. This call never blocks. The queue is bounded by
// setFrameQueueCapacity() purely as a memory/latency safety valve: if the
// worker falls behind far enough to hit that cap, only the single oldest
// still-waiting capture is dropped to admit the newest one (see
// getQueueOverflowDroppedCount()). Safe to call from any thread.
void pushFrame(AHardwareBuffer *ahb, int64_t timestampNs);

// Maximum number of not-yet-processed captures the frame queue holds before
// it starts dropping the oldest waiting entry (see pushFrame). Default 8,
// clamped to [1, 64]. Exposed as an overlay-configurable tunable like
...
// Total captures dropped so far because the frame queue was at capacity
// (see setFrameQueueCapacity). 0 under normal operation; a rising count
// means the worker is persistently unable to keep up at the configured
// queue depth.
