...
// User control for the backlog-pressure auto-bypass. Skips optional
// frame-generation work (real captures are always processed) when measured
// generation cost + queued work would exceed a latency budget. OFF by
// default — must be explicitly enabled by the user; see setDynamicBypassBudgetMultiplier
// to tune the budget once enabled, and getDynamicBypassCount for a diagnostic
// count of how often it has engaged.
...
// User control for the REAL-FRAME-FIRST policy: when enabled, a newer real
// capture already queued makes the current frame skip generation so capture
// throughput is never held up waiting on optional interpolation work. OFF by
// default — must be explicitly enabled by the user.
void setRealFrameFirstEnabled(bool enabled);

// User control for the generation-preflight safety-window check: rejects a
// generation whose projected cost would already eat too much of the source
// capture interval. OFF by default — must be explicitly enabled by the user;
// ratio (fraction of the capture interval, [0.1, 1.0]) is user-tunable.
void setPreflightSafetyWindowEnabled(bool enabled);
void setPreflightSafetyWindowRatio(double ratio);

// User control for genWorker's "dropping stale generated outputs" LOGI: logs
// every epoch-invalidated generation (bypass toggles, resizes, resolution
// changes can trigger many of these in a row). OFF by default — must be
// explicitly enabled by the user when diagnosing this.
void setStaleDropLoggingEnabled(bool enabled);

// User control for genWorkerThread's REAL->GEN ordering guard: OFF by
// default reproduces the original strict jobEpoch==currentEpoch behaviour
// (any epoch drift = dropped). Enabling this lets a generation still be
// published up to N epochs behind current — trading a bit of potential
// staleness for fewer dropped generations during rapid epoch churn.
// Epoch tolerance (epochs) is clamped to [0, 10].
