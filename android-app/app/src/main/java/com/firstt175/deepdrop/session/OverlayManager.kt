...
class OverlayManager(private val ctx: Context) {
    ...
    fun show(outputWidth: Int? = null, outputHeight: Int? = null) {
        ...
        layout.addView(
            ...
            FrameLayout.LayoutParams(
                ...
                // Fixed position below the fps/graph cluster regardless of whether
                // either of those is currently visible, so this line never has to
                // be repositioned when the user flips fps/graph on or off.
                topMargin = dp(HUD_STATS_TOP_DP)
                leftMargin = dp(HUD_MARGIN_DP)
            },
        )
    ...
    fun setStatsVisible(visible: Boolean) {
        statsView?.post { statsView?.visibility = if (visible) View.VISIBLE else View.GONE }
    }
    ...
    private fun relayoutHud() {
        ...
        statsView?.apply {
            ...
            setPadding(padH, padV, padH, padV)
            (layoutParams as? FrameLayout.LayoutParams)?.let { lp ->
                lp.topMargin = dp(HUD_STATS_TOP_DP)
                lp.leftMargin = margin
                layoutParams = lp
            }
        }
    ...
    companion object {
        ...
        // Below the fps text + frame graph cluster, at a fixed offset so it
        // never has to move when fps/graph are toggled independently.
        const val HUD_STATS_TOP_DP = 130
