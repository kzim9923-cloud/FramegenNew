...
class ShizukuCaptureEngine(
    ...
    private val frameCallback = object : IShizukuFrameCallback.Stub() {
        override fun onFrame(buffer: HardwareBuffer, timestampNs: Long) {
            // Capture has one job: forward the captured buffer to the native
            // render-loop queue. Scheduling, dropping, pacing and latency
            // policy are owned exclusively by RenderLoop.
            try {
        ...
        override fun onFrameMetrics(timestampNs: Long, frameTimeNs: Long, pacingJitterNs: Long) {
            // Intentionally ignored: capture must not perform timing/pacing or
            // latency decisions. RenderLoop owns frame scheduling.
        }
    }
