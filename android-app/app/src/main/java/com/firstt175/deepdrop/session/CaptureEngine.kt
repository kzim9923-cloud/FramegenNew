...
class CaptureEngine(
    ...
    fun setLsfgMode(width: Int, height: Int) {
        ...
        // Queue depth only needs to cover the gap between successive
        // ImageReader callbacks — ordering, backlog, and how many frames stay
        // in flight are the native frame queue's job now (see
        // NativeBridge.setFrameQueueCapacity), not this reader's. So this
        // reader always processes frames in arrival order (acquireNextImage)
        // at a single fixed depth; there's no separate "low latency" mode to
        // choose here since every frame ends up in the native queue anyway.
        val reader = ImageReader.newInstance(
            width, height, PixelFormat.RGBA_8888,
            /* maxImages */ lsfgQueueDepth,
            android.hardware.HardwareBuffer.USAGE_GPU_SAMPLED_IMAGE or
                    android.hardware.HardwareBuffer.USAGE_GPU_COLOR_OUTPUT
        )
        reader.setOnImageAvailableListener({ r ->
            // Just send: acquire the next frame and forward it to the native
            // pipeline. No latency/continuity decision-making here anymore —
            // the native frame queue owns that.
            val img = runCatching { r.acquireNextImage() }
