...
object AutoOverlayController {
    ...
    private fun onActivateRequested(ctx: Context) {
        ...
        mainHandler.post {
            settingsDrawer?.hide()
            // The launcher icon has already been shown by LauncherDotOverlay.
            // Do NOT create another ICON_BUTTON window here: doing so produces
            // two identical floating app icons. The settings window is only
            // the drawer/panel after the launcher entry has been tapped.
