...
import com.firstt175.deepdrop.session.diagnostics.AppIntegrity
import com.firstt175.deepdrop.session.diagnostics.CrashReporter
import com.firstt175.deepdrop.session.diagnostics.GamepadInputManager
...
class LsfgApplication : Application(), ImageLoaderFactory {
    ...
    // Wrapping the Application's own base context (rather than relying only on
    // each Activity's attachBaseContext) is what makes applicationContext.getString()
    // honor the user's AppLanguage choice too — needed because
    // AutoOverlayController passes ctx.applicationContext (not an Activity
    // context) into SettingsDrawerOverlay when the drawer is opened from the
    // floating launcher dot rather than from inside the foreground service.
