...
class SettingsDrawerOverlay(
    ...
    private fun buildPanel(): View {
        ...
        // Frame queue depth: every capture that fits in the native queue is
        // retained and always goes through generation — there is no bypass.
        // This only controls how many not-yet-processed captures the queue
        // will hold before it drops the single oldest waiting one (see
        // NativeBridge.setFrameQueueCapacity).
        val queueCapacityValue = TextView(ctx).apply {
            setTextColor(COLOR_PRIMARY)
            setTextSize(TypedValue.COMPLEX_UNIT_SP, 13f)
            typeface = android.graphics.Typeface.create(typeface, android.graphics.Typeface.BOLD)
            text = "${initial.frameQueueCapacity}"
        }
        frameGenSection.addView(
            sliderRow(
                labelText = "Frame queue depth",
                valueView = queueCapacityValue,
            ),
        )
        frameGenSection.addView(SeekBar(ctx).apply {
            max = 63
            progress = (initial.frameQueueCapacity - 1).coerceIn(0, 63)
            progressDrawable = buildSeekTrack()
            thumb = buildSeekThumb()
            splitTrack = false
            wireLiveDrag(
                label = "frameQueueCapacity",
                toValue = { p -> (p + 1).coerceIn(1, 64) },
                display = { q -> queueCapacityValue.text = "$q" },
                commit = { q ->
                    prefs.setFrameQueueCapacity(q)
                    runCatching { NativeBridge.setFrameQueueCapacity(q) }
                        .onFailure { Log.w(TAG, "setFrameQueueCapacity failed", it) }
                },
            )
        })
