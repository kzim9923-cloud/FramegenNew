...
import com.firstt175.deepdrop.prefs.FrameSchedulingMode
import java.io.File
...
class SettingsDrawerOverlay(
    ...
    private fun buildPanel(): View {
        ...
        // ---- Frame scheduling: exactly two modes -------------------------------
        frameGenSection.addView(sectionSpacer(8))
        frameGenSection.addView(miniHeader("Frame scheduling"))

        val schedulingButtons = mutableListOf<Button>()
        var selectedScheduling = initial.frameSchedulingMode
        val schedulingModes = listOf(
            FrameSchedulingMode.NO_WAIT to "ไม่รอเจน",
            FrameSchedulingMode.WAIT_GENERATION to "รอเจน",
        )
        val schedulingRow = LinearLayout(ctx).apply {
            orientation = LinearLayout.HORIZONTAL
            gravity = Gravity.CENTER_VERTICAL
            setPadding(0, dp(2), 0, dp(8))
        }
        fun paintSchedulingButtons() {
            schedulingButtons.forEachIndexed { index, button ->
                val selected = schedulingModes[index].first == selectedScheduling
        ...
        schedulingModes.forEach { (mode, label) ->
            val button = Button(ctx).apply {
                ...
                setOnClickListener {
                    selectedScheduling = mode
                    prefs.setFrameSchedulingMode(mode)
                    runCatching { NativeBridge.setFrameSchedulingMode(mode.ordinal) }
                        .onFailure { LsfgLog.w(TAG, "setFrameSchedulingMode failed", it) }
                    paintSchedulingButtons()
                }
            }
            schedulingButtons += button
            schedulingRow.addView(button, LinearLayout.LayoutParams(0, dp(36), 1f).apply {
                if (schedulingButtons.size > 1) leftMargin = dp(6)
            })
        }
        frameGenSection.addView(schedulingRow)
        paintSchedulingButtons()
        ...
        val imageEnhanceSection = collapsibleSection(panel, "IMAGE ENHANCEMENT / POST PROCESS")
        ...
        var ieStrength = iePrefs.load().imageEnhancementStrength
        ...
        imageEnhanceSection.addView(switchRow(
            ...
            NativeBridge.setImageEnhancement(it, ieBackend, ieStrength, ieContrast, ieSaturation)
        })
        ...
        listOf("CPU", "GPU / Vulkan").forEachIndexed { index, label ->
            val b = Button(ctx).apply {
                ...
                setOnClickListener {
                    ...
                    NativeBridge.setImageEnhancement(ieEnabled, ieBackend, ieStrength, ieContrast, ieSaturation)
        ...
        val ieStrengthValue = TextView(ctx).apply {
            setTextColor(COLOR_PRIMARY); setTextSize(TypedValue.COMPLEX_UNIT_SP, 13f)
            typeface = android.graphics.Typeface.DEFAULT_BOLD
            text = "${(ieStrength * 100).toInt()}%"
        }
        imageEnhanceSection.addView(sliderRow("Sharpness", ieStrengthValue))
        imageEnhanceSection.addView(SeekBar(ctx).apply {
            max = 100
            progress = (ieStrength * 100).toInt()
            progressDrawable = buildSeekTrack(); thumb = buildSeekThumb(); splitTrack = false
            wireLiveDrag("imageEnhancementStrength", { it / 100f }, { v -> ieStrengthValue.text = "${(v * 100).toInt()}%" }) { v ->
                ieStrength = v
                iePrefs.setImageEnhancementStrength(v)
                NativeBridge.setImageEnhancement(ieEnabled, ieBackend, ieStrength, ieContrast, ieSaturation)
            }
        })
