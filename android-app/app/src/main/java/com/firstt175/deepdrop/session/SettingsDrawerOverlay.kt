...
class SettingsDrawerOverlay(
    ...
    fun interface RefreshLayoutListener {
        fun onRefreshLayout()
    }
    ...
    private var refreshLayoutListener: RefreshLayoutListener? = null
    ...
    fun setRefreshLayoutListener(l: RefreshLayoutListener) { refreshLayoutListener = l }

    /** Re-read the live display metrics and snap the drawer/icon to the current screen. */
    fun refreshLayout() {
    ...
    private fun buildPanel(): View {
        ...
        val panel = LinearLayout(ctx).apply {
            orientation = LinearLayout.VERTICAL
            setPadding(dp(14), dp(12), dp(14), dp(78))
        }
        ...
        val refreshBtn = TextView(ctx).apply {
            ...
            isClickable = true
            isFocusable = true
            contentDescription = "Refresh layout"
            setOnClickListener {
                refreshLayout()
                refreshLayoutListener?.onRefreshLayout()
                animate().rotationBy(360f).setDuration(260L).start()
            }
        }
        ...
        frameGenSection.addView(miniHeader("Post process"))
        val postButtons = mutableListOf<Button>()
        var selectedPost = initial.postProcessMode
        fun paintPostButtons() {
            postButtons.forEachIndexed { index, button ->
                val mode = PostProcessMode.values()[index]
                val selected = mode == selectedPost
        ...
        val postRow = LinearLayout(ctx).apply {
            orientation = LinearLayout.HORIZONTAL
        }
        PostProcessMode.values().forEach { mode ->
            ...
            postButtons += b
            postRow.addView(b, LinearLayout.LayoutParams(0, dp(36), 1f).apply {
                marginEnd = dp(4)
            })
        }
        frameGenSection.addView(postRow)
        paintPostButtons()

        val sharpValue = TextView(ctx).apply {
            setTextColor(COLOR_PRIMARY)
            setTextSize(TypedValue.COMPLEX_UNIT_SP, 13f)
            typeface = android.graphics.Typeface.create(typeface, android.graphics.Typeface.BOLD)
            text = "${(initial.postProcessSharpness * 100f).toInt()}%"
        }
        frameGenSection.addView(sliderRow("Sharpness", sharpValue))
        frameGenSection.addView(SeekBar(ctx).apply {
            max = 20
            progress = (initial.postProcessSharpness * 20f).toInt().coerceIn(0, 20)
            progressDrawable = buildSeekTrack()
            thumb = buildSeekThumb()
            splitTrack = false
            wireLiveDrag(
                label = "postProcessSharpness",
                toValue = { p -> p / 20f },
                display = { v -> sharpValue.text = "${(v * 100f).toInt()}%" },
                commit = { v -> prefs.setPostProcessSharpness(v) },
            )
        })
