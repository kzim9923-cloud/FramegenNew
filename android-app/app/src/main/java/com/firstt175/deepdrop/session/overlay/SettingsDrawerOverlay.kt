...
class SettingsDrawerOverlay(
    ...
    private fun buildPanel(): View {
        ...
        var ieBackend = iePrefs.load().imageEnhancementBackend.ordinal
        ...
        imageEnhanceSection.addView(switchRow(
            ...
            NativeBridge.setImageEnhancement(it, ieBackend, ieContrast, ieSaturation)
        })

        val backendButtons = mutableListOf<Button>()
        fun paintIeBackendButtons() {
            backendButtons.forEachIndexed { index, b ->
                val selected = index == ieBackend
        ...
        listOf("CPU", "GPU / Vulkan").forEachIndexed { index, label ->
            val b = Button(ctx).apply {
                ...
                setOnClickListener {
                    ieBackend = index
                    iePrefs.setImageEnhancementBackend(if (index == 0) com.firstt175.deepdrop.prefs.ImageEnhancementBackend.CPU else com.firstt175.deepdrop.prefs.ImageEnhancementBackend.GPU)
                    NativeBridge.setImageEnhancement(ieEnabled, ieBackend, ieContrast, ieSaturation)
                    paintIeBackendButtons()
                }
            }
            backendButtons += b
            backendRow.addView(b, LinearLayout.LayoutParams(0, dp(36), 1f).apply {
                if (index > 0) leftMargin = dp(6)
            })
        }
        imageEnhanceSection.addView(miniHeader("Processing backend"))
        imageEnhanceSection.addView(backendRow)
        paintIeBackendButtons()
        ...
        upscaleSection.addView(switchRow(
            ...
            NativeBridge.setUpscaleEnabled(it)
        ...
        // ---- PRESENT MODE --------------------------------------------------------------
        // Controls the swapchain's VkPresentModeKHR. Applied live — the render loop
        // tears down and recreates the swapchain with the new mode on the next
        // present, no reinit needed. Falls back to FIFO natively if the surface
        ...
        listOf("Immediate", "Mailbox", "FIFO").forEachIndexed { index, label ->
            val b = Button(ctx).apply {
                ...
                setOnClickListener {
                    ...
                    NativeBridge.setPresentMode(chosen.vkValue)
                    paintPresentButtons()
                }
            }
            presentButtons += b
        ...
        // Zero-copy is exposed as experimental: the current LSFG Android context
        // binds input AHBs at creation, so native safely falls back to GPU copy.
        ...
        listOf("CPU Copy", "GPU Copy", "Zero Copy").forEachIndexed { index, label ->
            val b = Button(ctx).apply {
                ...
                setOnClickListener {
                    ...
                    NativeBridge.setFrameTransferMode(chosen.nativeValue)
        ...
        transferSection.addView(TextView(ctx).apply {
            text = "CPU Copy uses CPU memcpy. GPU Copy uses Vulkan transfer. Zero Copy is experimental and currently falls back to GPU Copy because LSFG binds its input AHBs when the context is created."
            setTextColor(COLOR_ON_SURFACE); alpha = 0.60f
            setTextSize(TypedValue.COMPLEX_UNIT_SP, 10f)
            setPadding(0, dp(4), 0, dp(8))
        })
        ...
        sessionPage.addView(sectionSpacer(20))
