...
class SettingsDrawerOverlay(
    ...
    private fun buildPanel(): View {
        ...
        // ---- ANTI-ALIASING --------------------------------------------------------------
        // Edge smoothing applied to source samples before contrast/saturation
        // enhancement and the selected upscale filter, inside the same GPU
        // compute pass. FXAA is a fast luma edge-detect + directional blend.
        // SMAA here is a lightweight single-pass edge-aware directional blur
        // approximating SMAA's distance-to-edge coverage idea — not the full
        // multi-pass SMAA algorithm with its area/search lookup textures,
        // which isn't practical inside this single compute pass.
        val aaSection = collapsibleSection(enhancementPage, "ANTI-ALIASING")
        val aaPrefs = LsfgPreferences(ctx)
        var aaMode = aaPrefs.load().antiAliasMode.ordinal

        val aaButtons = mutableListOf<Button>()
        fun paintAaButtons() {
            aaButtons.forEachIndexed { index, b ->
                val selected = index == aaMode
                b.setTextColor(if (selected) COLOR_PANEL_BG else COLOR_ON_SURFACE)
                (b.background as? GradientDrawable)?.setColor(if (selected) COLOR_PRIMARY else COLOR_CHIP_BG)
            }
        }
        val aaRow = LinearLayout(ctx).apply {
            orientation = LinearLayout.HORIZONTAL
            setPadding(0, dp(4), 0, dp(6))
        }
        listOf("Off", "FXAA", "SMAA").forEachIndexed { index, label ->
            val b = Button(ctx).apply {
                text = label
                isAllCaps = false
                setTextSize(TypedValue.COMPLEX_UNIT_SP, 12f)
                ...
                setOnClickListener {
                    aaMode = index
                    aaPrefs.setAntiAliasMode(com.firstt175.deepdrop.prefs.AntiAliasMode.entries[index])
                    queueNativeAction("antiAlias") {
                        NativeBridge.setAntiAliasMode(aaMode)
                    }
                    paintAaButtons()
                }
            }
            aaButtons += b
            aaRow.addView(b, LinearLayout.LayoutParams(0, dp(36), 1f).apply {
                if (index > 0) leftMargin = dp(6)
            })
        }
        aaSection.addView(miniHeader("Edge smoothing"))
        aaSection.addView(aaRow)
        paintAaButtons()
        aaSection.addView(TextView(ctx).apply {
            text = "Off = sharpest/cheapest, most jagged edges. FXAA = cheap edge smoothing. SMAA = wider, smoother edge-aware pass at a bit more GPU cost."
            setTextColor(COLOR_ON_SURFACE); alpha = 0.60f
            setTextSize(TypedValue.COMPLEX_UNIT_SP, 10f)
            setPadding(0, dp(4), 0, dp(8))
        })
        ...
        // ---- FRAME TRANSFER -------------------------------------------------------------
        // Selects how REAL capture AHB pixels enter the fixed LSFG input slots.
        // Each mode runs standalone — no fallback to another mode if it fails,
        // so a failure here means a frame is dropped rather than silently
        // switched to a different transfer path.
        val transferSection = collapsibleSection(overlayPage, "FRAME TRANSFER")
        val transferPrefs = LsfgPreferences(ctx)
        val transferValues = listOf(
            com.firstt175.deepdrop.prefs.FrameTransferMode.CPU_COPY,
            com.firstt175.deepdrop.prefs.FrameTransferMode.GPU_COPY,
            com.firstt175.deepdrop.prefs.FrameTransferMode.ZERO_COPY,
        )
        var transferIndex = transferValues.indexOf(transferPrefs.load().frameTransferMode).let { if (it < 0) 1 else it }
        val transferButtons = mutableListOf<Button>()
        fun paintTransferButtons() {
            transferButtons.forEachIndexed { index, b ->
                val selected = index == transferIndex
                b.setTextColor(if (selected) COLOR_PANEL_BG else COLOR_ON_SURFACE)
                (b.background as? GradientDrawable)?.setColor(if (selected) COLOR_PRIMARY else COLOR_CHIP_BG)
            }
        }
        val transferRow = LinearLayout(ctx).apply {
            orientation = LinearLayout.HORIZONTAL
            setPadding(0, dp(4), 0, dp(6))
        }
        listOf("CPU Copy", "GPU Copy", "Zero Copy").forEachIndexed { index, label ->
            val b = Button(ctx).apply {
                text = label
                isAllCaps = false
                setTextSize(TypedValue.COMPLEX_UNIT_SP, 12f)
                ...
                setOnClickListener {
                    transferIndex = index
                    val chosen = transferValues[index]
                    transferPrefs.setFrameTransferMode(chosen)
                    queueNativeAction("frameTransferMode") { NativeBridge.setFrameTransferMode(chosen.nativeValue) }
                    paintTransferButtons()
                }
            }
            transferButtons += b
            transferRow.addView(b, LinearLayout.LayoutParams(0, dp(36), 1f).apply {
                if (index > 0) leftMargin = dp(6)
            })
        }
        transferSection.addView(miniHeader("REAL frame → framegen input"))
        transferSection.addView(transferRow)
        paintTransferButtons()
        transferSection.addView(TextView(ctx).apply {
            text = "CPU Copy uses CPU memcpy. GPU Copy uses Vulkan transfer. Zero Copy skips copying entirely and hands the capture buffer straight to framegen. Each mode runs on its own — none of them fall back to another if it fails."
            setTextColor(COLOR_ON_SURFACE); alpha = 0.60f
            setTextSize(TypedValue.COMPLEX_UNIT_SP, 10f)
            setPadding(0, dp(4), 0, dp(8))
        })

        overlayPage.addView(divider())
