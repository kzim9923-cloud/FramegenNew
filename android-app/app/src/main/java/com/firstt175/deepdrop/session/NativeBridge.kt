...
object NativeBridge {
    ...
    /**
     * Sets the maximum number of not-yet-processed captures the native frame
     * queue holds before it starts dropping the single oldest waiting entry
     * to admit new captures. Every frame that stays in the queue always goes
     * through frame generation — there is no load-based bypass. Clamped to
     * [1, 64] natively. Safe to call from any thread.
     */
    external fun setFrameQueueCapacity(capacity: Int)

    /** Current frame queue capacity (see [setFrameQueueCapacity]). */
    external fun getFrameQueueCapacity(): Int

    /**
     * Total captures dropped so far because the frame queue was at capacity.
     * 0 under normal operation; a rising count means the worker is
     * persistently unable to keep up at the configured queue depth.
     */
    external fun getQueueOverflowDroppedCount(): Long
