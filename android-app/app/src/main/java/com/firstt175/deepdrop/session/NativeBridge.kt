...
object NativeBridge {
     ...
     */
    /** 0 = do not wait for generation; 1 = wait for every generation. */
    external fun setFrameSchedulingMode(mode: Int)
    ...
    /**
     * Enables/disables the backlog-pressure auto-bypass. OFF by default:
     * generation is never silently skipped for queue pressure until the user
     * turns this on from Settings. When on, [setDynamicBypassBudgetMultiplier]
     * tunes how aggressively it engages.
     */
    external fun setDynamicBypassEnabled(enabled: Boolean)

    /**
     * Tunes the backlog-pressure auto-bypass (only has an effect while it's
     * enabled via [setDynamicBypassEnabled]). Multiplies the source capture
     * interval to form the queued-work latency budget; clamped natively to
     * [1.0, 5.0]. Lower = bypasses sooner under load, higher = more tolerant.
     */
    external fun setDynamicBypassBudgetMultiplier(multiplier: Double)

    /** Diagnostic: how many times the backlog-pressure auto-bypass has engaged since init. */
