...
data class LsfgConfig(
    ...
    val multiplier: Int,
    val flowScale: Float,
    val postProcessMode: PostProcessMode,
    val postProcessSharpness: Float,
...
enum class PostProcessMode(val prefValue: String, val nativeValue: Int, val label: String) {
    OFF("off", 0, "Off"),
    CAS("cas", 1, "CAS"),
    RCAS("rcas", 2, "RCAS");

    companion object {
        fun fromPref(value: String?): PostProcessMode =
...
class LsfgPreferences(ctx: Context) {
    ...
    fun load(): LsfgConfig = LsfgConfig(
        ...
        postProcessMode = PostProcessMode.fromPref(prefs.getString(KEY_POST_PROCESS_MODE, "off")),
        postProcessSharpness = prefs.getFloat(KEY_POST_PROCESS_SHARPNESS, 0.65f).coerceIn(0f, 1f),
    ...
    fun setPostProcessMode(value: PostProcessMode) = prefs.edit().putString(KEY_POST_PROCESS_MODE, value.prefValue).apply()
    fun setPostProcessSharpness(value: Float) = prefs.edit().putFloat(KEY_POST_PROCESS_SHARPNESS, value.coerceIn(0f, 1f)).apply()
    ...
    companion object {
        ...
        private const val KEY_POST_PROCESS_MODE = "post_process_mode"
        private const val KEY_POST_PROCESS_SHARPNESS = "post_process_sharpness"
