...
data class LsfgConfig(
    ...
    /**
     * Maximum number of not-yet-processed captures the native frame queue
     * (see [com.firstt175.deepdrop.session.NativeBridge.setFrameQueueCapacity])
     * holds before it starts dropping the oldest waiting entry. Every frame
     * that stays in the queue always goes through frame generation — there
     * is no bypass. Higher values absorb longer worker stalls at the cost
     * of more retained buffer memory and latency if the worker stays behind.
     */
...
class LsfgPreferences(ctx: Context) {
    ...
    fun load(): LsfgConfig = LsfgConfig(
        ...
        frameQueueCapacity = prefs.getInt(KEY_FRAME_QUEUE_CAPACITY, DEFAULT_FRAME_QUEUE_CAPACITY),
    ...
    fun setCaptureSource(value: CaptureSource) = prefs.edit()
        ...
        .apply()
    fun setFrameQueueCapacity(value: Int) =
        prefs.edit().putInt(KEY_FRAME_QUEUE_CAPACITY, value.coerceIn(1, 64)).apply()
    ...
    companion object {
        ...
        private const val KEY_FRAME_QUEUE_CAPACITY = "frame_queue_capacity"
        const val DEFAULT_FRAME_QUEUE_CAPACITY = 8
