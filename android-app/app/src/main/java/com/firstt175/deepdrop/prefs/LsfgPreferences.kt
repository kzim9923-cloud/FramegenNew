...
data class LsfgConfig(
    ...
    val antiAliasMode: AntiAliasMode,
) {
}
...
/** Edge anti-aliasing applied before enhancement/upscale. OFF=0, FXAA=1,
 *  SMAA=2 (lightweight single-pass approximation — native side ordinal). */
enum class AntiAliasMode(val prefValue: String) {
    OFF("off"),
    FXAA("fxaa"),
    SMAA("smaa");

    companion object {
        fun fromPref(value: String?): AntiAliasMode =
...
class LsfgPreferences(ctx: Context) {
    ...
    fun load(): LsfgConfig = LsfgConfig(
        ...
        antiAliasMode = AntiAliasMode.fromPref(prefs.getString(KEY_ANTI_ALIAS_MODE, null)),
    )
    ...
    fun setAntiAliasMode(value: AntiAliasMode) = prefs.edit()
        .putString(KEY_ANTI_ALIAS_MODE, value.prefValue).apply()
    ...
    companion object {
        ...
        private const val KEY_ANTI_ALIAS_MODE = "anti_alias_mode"
