...
data class LsfgConfig(
    ...
    val frameSchedulingMode: FrameSchedulingMode,
...
enum class FrameSchedulingMode(val prefValue: String) {
    NO_WAIT("no_wait"),
    WAIT_GENERATION("wait_generation");

    companion object {
        fun fromPref(value: String?): FrameSchedulingMode =
            entries.firstOrNull { it.prefValue == value } ?: NO_WAIT
    }
}
...
/**
 * Bundles the three frame-gen admission-safety toggles
 * ([LsfgPreferences.isDynamicBypassEnabled], [LsfgPreferences.isRealFrameFirstEnabled],
 * [LsfgPreferences.isPreflightSafetyWindowEnabled]) into a single switch so they
 * can't be left in a half-on state by accident.
 *
 *  - [OFF]: applying this preset force-writes all three to false — even if one
 *    had been hand-enabled before. "Off" always really means none of the three
 *    guards are active.
 *  - [STRICT]: applying this preset force-writes all three to true.
 *  - [CUSTOM]: has no fixed values of its own. It only exists so the UI can
 *    show "custom" as selected after the user hand-toggles one of the three
 *    switches individually instead of using OFF/STRICT.
 *
 * This key only remembers which button should show selected; it does not
 * drive native behavior by itself; see [LsfgPreferences.setFramegenGuardPreset].
 */
enum class FramegenGuardPreset(val prefValue: String) {
    OFF("off"),
    STRICT("strict"),
    CUSTOM("custom");

    companion object {
        fun fromPref(value: String?): FramegenGuardPreset =
...
class LsfgPreferences(ctx: Context) {
    ...
    fun load(): LsfgConfig = LsfgConfig(
        ...
        frameSchedulingMode = FrameSchedulingMode.fromPref(prefs.getString(KEY_FRAME_SCHEDULING_MODE, null)),
    ...
    // --- Auto-bypass policies (backlog-pressure "dynamic bypass" and
    // "real-frame-first") — both OFF by default. Neither is part of
    // LsfgConfig/initContext: they're hot-applied via NativeBridge setters
    // (like pacing params), so standalone get/set pairs are enough here.

    /** Backlog-pressure auto-bypass: off unless the user explicitly turns it on. */
    fun isDynamicBypassEnabled(): Boolean = prefs.getBoolean(KEY_DYNAMIC_BYPASS_ENABLED, false)
    fun setDynamicBypassEnabled(value: Boolean) =
        prefs.edit().putBoolean(KEY_DYNAMIC_BYPASS_ENABLED, value).apply()

    /** Multiplier applied to the capture interval to form the dynamic-bypass latency budget. */
    fun getDynamicBypassBudgetMultiplier(): Double =
        prefs.getFloat(KEY_DYNAMIC_BYPASS_BUDGET, 1.5f).toDouble().coerceIn(1.0, 5.0)
    fun setDynamicBypassBudgetMultiplier(value: Double) = prefs.edit()
        .putFloat(KEY_DYNAMIC_BYPASS_BUDGET, value.coerceIn(1.0, 5.0).toFloat())
        .apply()

    /** REAL-FRAME-FIRST auto-bypass: off unless the user explicitly turns it on. */
    fun isRealFrameFirstEnabled(): Boolean = prefs.getBoolean(KEY_REAL_FRAME_FIRST_ENABLED, false)
    fun setRealFrameFirstEnabled(value: Boolean) =
        prefs.edit().putBoolean(KEY_REAL_FRAME_FIRST_ENABLED, value).apply()

    /** Generation-preflight safety-window check: off unless the user explicitly turns it on. */
    fun isPreflightSafetyWindowEnabled(): Boolean =
        prefs.getBoolean(KEY_PREFLIGHT_SAFETY_WINDOW_ENABLED, false)
    fun setPreflightSafetyWindowEnabled(value: Boolean) =
        prefs.edit().putBoolean(KEY_PREFLIGHT_SAFETY_WINDOW_ENABLED, value).apply()

    /** Fraction of the capture interval a projected generation may consume before rejection. */
    fun getPreflightSafetyWindowRatio(): Double =
        prefs.getFloat(KEY_PREFLIGHT_SAFETY_WINDOW_RATIO, 0.65f).toDouble().coerceIn(0.1, 1.0)
    fun setPreflightSafetyWindowRatio(value: Double) = prefs.edit()
        .putFloat(KEY_PREFLIGHT_SAFETY_WINDOW_RATIO, value.coerceIn(0.1, 1.0).toFloat())
        .apply()

    /**
     * genWorker's "dropping stale generated outputs" logcat line: off unless
     * the user explicitly turns it on. Fires on every epoch-invalidated
     * generation (bypass toggles, resizes, resolution changes), which can
     * spam logcat under normal operation, so it stays quiet by default.
     */
    fun isStaleDropLoggingEnabled(): Boolean =
        prefs.getBoolean(KEY_STALE_DROP_LOGGING_ENABLED, false)
    fun setStaleDropLoggingEnabled(value: Boolean) =
        prefs.edit().putBoolean(KEY_STALE_DROP_LOGGING_ENABLED, value).apply()

    /**
     * REAL->GEN ordering guard tolerance: off unless the user explicitly
     * turns it on. Off means the original strict jobEpoch==currentEpoch
     * behaviour (every epoch-stale generation dropped). On lets a
     * generation publish up to [getStaleGenerationToleranceEpochs] epochs
     * behind current instead of always being dropped.
     */
    fun isStaleGenerationToleranceEnabled(): Boolean =
        prefs.getBoolean(KEY_STALE_GEN_TOLERANCE_ENABLED, false)
    fun setStaleGenerationToleranceEnabled(value: Boolean) =
        prefs.edit().putBoolean(KEY_STALE_GEN_TOLERANCE_ENABLED, value).apply()

    /** Epochs of drift tolerated before a generation is dropped; only applies once enabled. */
    fun getStaleGenerationToleranceEpochs(): Int =
        prefs.getInt(KEY_STALE_GEN_TOLERANCE_EPOCHS, 0).coerceIn(0, 10)
    fun setStaleGenerationToleranceEpochs(value: Int) =
        prefs.edit().putInt(KEY_STALE_GEN_TOLERANCE_EPOCHS, value.coerceIn(0, 10)).apply()

    /**
     * Which [FramegenGuardPreset] the guard-preset row should show as selected.
     * Defaults to OFF, matching the individual defaults above (all-false),
     * so existing installs that never touch this new row read back
     * correctly as "off" with no migration needed.
     */
    fun getFramegenGuardPreset(): FramegenGuardPreset =
        FramegenGuardPreset.fromPref(prefs.getString(KEY_FRAMEGEN_GUARD_PRESET, null))
    fun setFramegenGuardPreset(value: FramegenGuardPreset) =
        prefs.edit().putString(KEY_FRAMEGEN_GUARD_PRESET, value.prefValue).apply()
    ...
    fun getFrameSchedulingMode(): FrameSchedulingMode =
        FrameSchedulingMode.fromPref(prefs.getString(KEY_FRAME_SCHEDULING_MODE, null))

    fun setFrameSchedulingMode(value: FrameSchedulingMode) =
        prefs.edit().putString(KEY_FRAME_SCHEDULING_MODE, value.prefValue).apply()
    ...
    companion object {
        ...
        private const val KEY_FRAME_SCHEDULING_MODE = "frame_scheduling_mode"
        ...
        private const val KEY_DYNAMIC_BYPASS_ENABLED = "dynamic_bypass_enabled"
        private const val KEY_DYNAMIC_BYPASS_BUDGET = "dynamic_bypass_budget_multiplier"
        private const val KEY_REAL_FRAME_FIRST_ENABLED = "real_frame_first_enabled"
        private const val KEY_PREFLIGHT_SAFETY_WINDOW_ENABLED = "preflight_safety_window_enabled"
        private const val KEY_PREFLIGHT_SAFETY_WINDOW_RATIO = "preflight_safety_window_ratio"
        private const val KEY_STALE_DROP_LOGGING_ENABLED = "stale_drop_logging_enabled"
        private const val KEY_STALE_GEN_TOLERANCE_ENABLED = "stale_gen_tolerance_enabled"
        private const val KEY_STALE_GEN_TOLERANCE_EPOCHS = "stale_gen_tolerance_epochs"
        private const val KEY_FRAMEGEN_GUARD_PRESET = "framegen_guard_preset"
