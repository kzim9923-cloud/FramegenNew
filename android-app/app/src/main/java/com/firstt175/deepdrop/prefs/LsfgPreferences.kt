...
data class LsfgConfig(
    ...
    val imageEnhancementBackend: ImageEnhancementBackend,
    val imageEnhancementStrength: Float,
...
enum class ImageEnhancementBackend(val prefValue: String) {
    CPU("cpu"),
    GPU("gpu");

    companion object {
        fun fromPref(value: String?): ImageEnhancementBackend =
            entries.firstOrNull { it.prefValue == value } ?: CPU
    }
}
...
class LsfgPreferences(ctx: Context) {
    ...
    fun load(): LsfgConfig = LsfgConfig(
        ...
        imageEnhancementBackend = ImageEnhancementBackend.fromPref(prefs.getString(KEY_IMAGE_ENHANCEMENT_BACKEND, null)),
        imageEnhancementStrength = prefs.getFloat(KEY_IMAGE_ENHANCEMENT_STRENGTH, 0.35f).coerceIn(0f, 1f),
    ...
    fun setImageEnhancementBackend(value: ImageEnhancementBackend) = prefs.edit()
        .putString(KEY_IMAGE_ENHANCEMENT_BACKEND, value.prefValue).apply()
    fun setImageEnhancementStrength(value: Float) = prefs.edit()
        .putFloat(KEY_IMAGE_ENHANCEMENT_STRENGTH, value.coerceIn(0f, 1f)).apply()
    ...
    companion object {
        ...
        private const val KEY_IMAGE_ENHANCEMENT_BACKEND = "image_enhancement_backend"
        private const val KEY_IMAGE_ENHANCEMENT_STRENGTH = "image_enhancement_strength"
